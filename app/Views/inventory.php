...
<section class="metric-grid metric-grid-5">
    ...
    <article class="metric-card accent-green"><div class="metric-top"><span class="metric-icon"><?= Ui::icon('trend') ?></span></div><span class="metric-label">هامش الربح المتوقع</span><strong><?= Ui::money($summary['potential_profit']) ?></strong><small>فرق البيع والتكلفة للمخزون</small></article>
...
<section class="panel table-panel">
    ...
    <?php if ($products): ?><div class="table-wrap"><table class="data-table"><thead><tr><th><?= $sortLink('name', 'المنتج') ?></th><th><?= $sortLink('category', 'التصنيف') ?></th><th><?= $sortLink('stock', 'المخزون') ?></th><th>حد الطلب</th><th><?= $sortLink('price', 'سعر البيع') ?></th><th><?= $sortLink('value', 'قيمة المخزون') ?></th><th>هامش الربح %</th><th>الحالة</th></tr></thead><tbody>
    <?php foreach ($products as $product): ?><?php $marginPct = ((float) $product['price']) > 0 ? (((float) $product['price'] - (float) $product['cost']) / (float) $product['price']) * 100 : null; ?><tr class="<?= $product['out'] ? 'row-danger' : ($product['low'] ? 'row-warning' : '') ?>"><td><div class="product-cell"><span><?= Ui::e(Ui::slice($product['name'], 1)) ?></span><div><strong><a href="<?= Ui::e(Ui::url('/products/view', ['id' => $product['id']])) ?>"><?= Ui::e($product['name']) ?></a></strong><small>تكلفة <?= Ui::money($product['cost']) ?></small></div></div></td><td><span class="category-pill"><?= Ui::e($product['category']) ?></span></td><td><strong><?= Ui::number($product['stock_qty']) ?></strong></td><td><?= Ui::number($product['reorder_level']) ?></td><td class="money-cell"><?= Ui::money($product['price']) ?></td><td class="money-cell"><?= Ui::money($product['stock_value']) ?></td><td><?= Ui::percent($marginPct) ?></td><td><?php if ($product['out']): ?><span class="badge danger"><i></i> نفد</span><?php elseif ($product['low']): ?><span class="badge warning"><i></i> منخفض</span><?php else: ?><span class="badge success"><i></i> متوفر</span><?php endif; ?></td></tr><?php endforeach; ?>
